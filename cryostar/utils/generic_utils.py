...
class Progbar:
    ...
    def __init__(
        ...
        self._dynamic_display = ((hasattr(sys.stdout, "isatty") and sys.stdout.isatty()) or "ipykernel" in sys.modules
                                 or "posix" in sys.modules or "PYCHARM_HOSTED" in os.environ)
        self._total_width = 0
        self._seen_so_far = 0
...
def download_file(url, dst_file_path):
    ...
    try:
        try:
            urlretrieve(url, dst_file_path, DLProgbar())
...
def get_file(
    ...
    cache_dir `~/CACHE_DIR_NAME`, placed in the cache_subdir `datasets`,
    and given the filename `fname`. The final location of a file
    `example.txt` would therefore be `~/CACHE_DIR_NAME/datasets/example.txt`.
    ...
    Args:
        ...
        cache_dir: Location to store cached files, when None it
            defaults to the default directory `~/CACHE_DIR_NAME/`.
    Returns:
    ...
    if os.path.exists(fpath):
        ...
        if file_hash is not None:
            if not validate_file(fpath, file_hash, algorithm=hash_algorithm):
                print_log("A local file was found, but it seems to be "
    ...
    if download:
        print_log(f"Downloading data from {origin}")
